...
### 🔥Task1 选取五只股票进行有效前沿的绘制以及给定预期收益的最佳持仓配比求解
...
σ_p^2 = Σ (i=1 to n) Σ (j=1 to n) (w_i * w_j * σ_ij)

#### 约束条件  

1. 所有资产权重之和为 1（表示所有资金都被投资）：  

Σ (i=1 to n) w_i = 1

2. 投资组合的预期回报等于目标回报 \( R_t \)：  

E(R_p) = Σ (i=1 to n) (w_i * E(R_i)) = R_t
